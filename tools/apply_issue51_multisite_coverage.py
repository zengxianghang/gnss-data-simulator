...
new_test = r'''TEST(V1Acceptance, EveryFrozenSignalRunsTruthStateCodeAndDopplerResidualChecks) {
    ...
    // The three sites intentionally form a coverage union. A single ground
    // point cannot see J04, G17, C22 and C24 simultaneously at this epoch.
    // G17 and C22 coordinates are their broadcast-orbit subpoints at the
    // compact test start epoch, derived from the same RINEX4 ephemerides.
    ...
    for (const ResidualSite& site : sites) {
        ...
        for (const char* required :
             {"gps_week", "sow_sec", "signal_id", "signal_name", "satellite_number", "wavelength_m",
              ...
              "receiver_vz_mps", "cn0_dbhz", "broadcast_message_family", "pseudorange_valid", "doppler_valid",
              "pseudorange_m", "doppler_hz"}) {
        ...
        while (std::getline(input, line)) {
            ...
            if (fields[column.at("pseudorange_valid")] == "1") {
                ...
                EXPECT_EQ(code_status, 0) << "site=" << site.name
                                          << " invalid code must stay explicitly unavailable; signal=" << signal_name;
    ...
    for (std::size_t index = 0; index < definition_count; ++index) {
        ...
        if (signal_name == "Galileo E6") {
            ...
            EXPECT_GT(signal_stats.code_residuals, 0U)
                << "real compact coverage union must exercise every non-E6 signal: " << signal_name;
        }
    }
    std::fprintf(stderr, "CODE_COVERAGE_UNION covered=%zu total=21 expected_before_has=20\n",
                 code_covered_signal_count);
...
print("multi-site real-ephemeris residual coverage union applied")
